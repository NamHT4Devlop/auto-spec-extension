...
function parseTsImports(srcDir) {
    const deps = new Map();
    ...
    for (const file of walk(srcDir)) {
        ...
        while ((m = re.exec(content)) !== null) {
            ...
            const resolved = path.relative(srcDir, path.resolve(path.dirname(file), m[1])).replace(/\\/g, '/');
...
function inferLayer(relPath) {
    if (relPath.includes('workflow/')) {
        return 'workflow';
    }
    if (relPath.includes('utils/')) {
        return 'utils';
    }
    if (relPath.includes('constants/') || relPath.includes('types/')) {
        return 'domain';
    }
    if (relPath === 'extension' || relPath === 'logger') {
        return 'commands';
    }
    return 'utils';
}
function inferType(relPath, layer) {
    if (layer === 'commands') {
        return 'command';
    }
    if (layer === 'workflow') {
    ...
    if (relPath.includes('storage') || relPath.includes('checkpoint')) {
        return 'storage';
    }
    if (layer === 'domain') {
        return 'domain';
...
function buildFileGraph(srcDir) {
