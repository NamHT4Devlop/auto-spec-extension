...
exports.updateKBStandalone = updateKBStandalone;
...
async function updateKBStandalone(workspaceRoot, model, token, progress) {
    // 1. Read config
    ...
    if (!fs.existsSync(kbPath)) {
        vscode.window.showErrorMessage(`Auto Spec Kit: Knowledge Base not found at "${kbRelPath}". Please run "Generate Knowledge Base" first.`);
        return;
    }
    ...
    (0, logger_1.kbHeader)(1, 1, 'UPDATE KNOWLEDGE BASE');
    (0, logger_1.log)(`ℹ  Description: ${userDescription}`);
    ...
    try {
        ...
        gitDiff = stdout.trim();
        if (gitDiff) {
            (0, logger_1.log)(`ℹ  Git diff (changed files):\n${gitDiff}`);
        }
    }
    catch {
        // git not available or no commits — that's fine
        (0, logger_1.log)('ℹ  Git diff not available — proceeding with user description only');
    }
    // 5. Load KB
...
⚠️ Only output files that ACTUALLY changed. If nothing is new: "(no update needed)".`;
    const deltaResult = await (0, copilot_1.callCopilot)(model, SYSTEM, deltaPrompt, token, 'Update KB Delta');
    ...
    if (updatedFiles.length === 0) {
        (0, logger_1.log)('ℹ  No KB files needed updating');
        ...
        const action = await vscode.window.showInformationMessage(`📚 Knowledge Base updated: ${updatedFiles.length} file(s) — ${updatedFiles.join(', ')}`, 'View updated file');
