...
import { loadKnowledgeBase, extractFiles, saveFile } from '../utils/file-utils';
...
function loadReviewSkills(workspaceRoot: string, kbRelPath: string, extensionPath: string): string {
  ...
  if (fs.existsSync(universalPath)) {
    ...
    log(`✅ Review skills loaded from extension resources: review-skills-universal.md (${(content.length / 1024).toFixed(1)}KB)`);
    log(`   ⚠  No KB found → Section 14 (project-specific rules) is missing. Run "Generate KB" for a full review.`);
    return content;
  }
...
⚠️ Do NOT generate code in this step.`, 'Planning');
  ...
  saveFile(sessionDir, '01-plan/plan.md', `# Implementation Plan\n\n**Requirement:** ${requirement}\n\n${plan01}`);
  log('✅ Saved → 01-plan/plan.md');
...
### 🎯 VERDICT: APPROVED / NEEDS_REVISION (+ one-line reason)`, 'Review Plan');
  ...
  saveFile(sessionDir, '02-plan-review/review.md', `# Plan Review\n\n${planReview}`);
  log('✅ Saved → 02-plan-review/review.md');

  // ══════════════════════════════════════════════════════════════
  // STEP 03 — COMMENT & FINALIZE PLAN
  // ══════════════════════════════════════════════════════════════
  step(3, 'PLAN FEEDBACK CHECKPOINT');
  const planComment = await askComment('Plan');

  let planFinal: string;
  if (planComment) {
    log(`ℹ  Updating plan with feedback: "${planComment}"`);
    planFinal = await ai(`\
...
→ Generate the COMPLETE improved plan.
  Mark [UPDATED] before each line changed from the original plan.`, 'Update Plan');
    saveFile(sessionDir, '01-plan/plan-final.md', `# Final Plan\n\n**Feedback:** ${planComment}\n\n${planFinal}`);
  } else {
    planFinal = plan01;
    saveFile(sessionDir, '01-plan/plan-final.md', `# Final Plan\n\n${plan01}`);
    log('✅ Plan finalized (no changes)');
  }
...
Order: entity/model → repository/DAO → service → controller → DTO/schema`, 'Generate Code');
  ...
  saveFile(sessionDir, '03-code/code-raw.md', code04);
  log('✅ Saved → 03-code/code-raw.md');
  ...
  log('ℹ  Loading git context for review...');
  const newFilePaths = extractFiles(code04).map(f => f.filePath);
  const gitCtx = loadGitContext(workspaceRoot, newFilePaths);
  const gitContextBlock = formatGitContextForPrompt(gitCtx);
...
7. If Section 14 (Project-Specific Rules) exists in Review Skills, enforce with highest priority.`;
  ...
  const codeReview = await callCopilot(model, reviewSystemPrompt, `\
...
→ Regenerate ALL fixed code.
  ...
  Only fix the issues identified, do not change what is already correct.`, 'Fix Code');
    saveFile(sessionDir, '03-code/code-final.md', codeFinal);
  } else {
    codeFinal = code04;
    saveFile(sessionDir, '03-code/code-final.md', code04);
    log('✅ Code approved (no changes needed)');
  }
...
AFTER: implement each case.`, 'Write Tests');
  ...
  saveFile(sessionDir, '05-tests/tests-raw.md', tests07);
  log('✅ Saved → 05-tests/tests-raw.md');
...
### 🎯 VERDICT: APPROVED / NEEDS_MORE_TESTS`, 'Review Tests');
  ...
  saveFile(sessionDir, '06-test-review/review.md', `# Test Review\n\n${testsReview}`);
  log('✅ Saved → 06-test-review/review.md');

  // ══════════════════════════════════════════════════════════════
  // STEP 09 — COMMENT & FINALIZE TESTS
  // ══════════════════════════════════════════════════════════════
  step(9, 'TEST FEEDBACK CHECKPOINT');
  const testsComment = await askComment('Tests');
  const testsNeedMore = /NEEDS_MORE_TESTS|Missing test|\[CRITICAL\]/.test(testsReview);

  let testsFinal: string;
  if (testsComment || testsNeedMore) {
    log('ℹ  Improving tests ...');
    testsFinal = await ai(`\
...
→ Regenerate the COMPLETE improved test suite.
  ...
  Keep the ### FILE: <path> format.`, 'Update Tests');
    saveFile(sessionDir, '05-tests/tests-final.md', testsFinal);
  } else {
    testsFinal = tests07;
    saveFile(sessionDir, '05-tests/tests-final.md', tests07);
    log('✅ Tests approved (no changes)');
  }
  ...
  if (applyToProject) {
    for (const f of allFiles) {
      ...
      fs.mkdirSync(path.dirname(dest), { recursive: true });
      fs.writeFileSync(dest, f.code, 'utf-8');
      log(`✅ [PROJECT] Applied → ${f.filePath}`);
    }
    // Refresh VS Code explorer
    vscode.commands.executeCommand('workbench.files.action.refreshFilesExplorer');
  } else {
    log(`ℹ  Files saved to session only: ${sessionDir}/08-files/`);
  }
  ...
  if (!testCmd) {
    log('⚠  autoSpecKit.testCommand is not configured');
    log('   → Go to Settings → Extensions → Auto Spec Kit → Test Command');
    log('   → Example: npx jest --coverage --coverageReporters=text 2>&1');
  } else {
    log(`ℹ  Command : ${testCmd}`);
    log(`ℹ  Cwd     : ${workspaceRoot}`);
    const t0 = Date.now();
    try {
  ...
  if (testResult.output) {
    saveFile(sessionDir, '07-evidence/test-output.txt', testResult.output);
    log('✅ Saved → 07-evidence/test-output.txt');
  }
...
`;
  ...
  saveFile(sessionDir, '07-evidence/EVIDENCE.md', evidence);
  log('✅ Saved → 07-evidence/EVIDENCE.md');

  saveFile(sessionDir, 'README.md', `\
# Session: ${path.basename(sessionDir)}
...
`);
  ...
  if (kbUpdateChoice?.update) {
    log('ℹ  Analyzing delta to update KB ...');
...
⚠️ Only output files that ACTUALLY changed. If nothing is new, write "(no update needed)".`, 'Update Knowledge Base');
    ...
    log(kbDelta);
    saveFile(sessionDir, '09-kb-updates/kb-delta.md', `# KB Update Delta\n\n**Task:** ${requirement}\n**Date:** ${now}\n\n${kbDelta}`);
